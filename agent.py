...
openai_client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"), timeout=180)
...
mongo_db = mongo_client["email_agent"]
...
def already_processed(msg_id):
    return emails_collection.find_one({"id": msg_id}) is not None
...
def analyze_node(state: EmailState):
    ...
    try:
        ...
        system_prompt = """
You are an AI email assistant. Classify emails into:
Urgent, Meeting, Finance, Personal, Low Priority.
...
def gmail_request_with_retry(func, *args, retries=5, initial_delay=2, **kwargs):
    ...
    for attempt in range(retries):
        ...
        try:
            ...
            print(
                f"[WARN] Gmail request failed attempt {attempt+1}: {e}. Retrying in {delay}s..."
            )
